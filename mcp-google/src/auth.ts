...
export function createOAuth2Client(): Auth.OAuth2Client {
  ...
  if (refreshToken) {
    ...
    throw new Error(
      "ไม่พบ GOOGLE_REFRESH_TOKEN — รัน: node dist/index.js --auth เพื่อรับ Token"
    );
  }
...
export async function validateAuth(): Promise<void> {
  ...
  if (auth instanceof google.auth.GoogleAuth) {
    const client = await (auth as Auth.GoogleAuth).getClient();
    await (client as Auth.OAuth2Client).getAccessToken();
  } else {
    await (auth as Auth.OAuth2Client).getAccessToken();
  }
}
...
export async function runAuthFlow(): Promise<void> {
  ...
  if (!clientId || !clientSecret) {
    console.error(
      "❌ ต้องมี GOOGLE_CLIENT_ID และ GOOGLE_CLIENT_SECRET ใน .env\n" +
