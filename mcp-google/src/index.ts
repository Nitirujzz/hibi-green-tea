...
server.tool(
  "check_google_auth",
  "ตรวจสอบว่าเชื่อมต่อ Google ได้หรือไม่ และแสดงวิธีการ auth ที่ใช้อยู่",
  {},
  async () => {
    ...
    if (serviceKeyPath || serviceKeyJson) method = "✅ Service Account (แนะนำสำหรับ Remote)";
    else if (hasOAuth) method = "✅ OAuth2 + Refresh Token";
...
server.tool(
  "setup_hibi_drive",
  "สร้างโครงสร้าง Folder ทั้งหมดสำหรับ Hibi Matcha ใน Google Drive (9 หมวดหลัก)",
  {},
  async () => {
...
server.tool(
  "create_google_doc",
  "สร้าง Google Document ใหม่ใน Folder ที่กำหนด",
  {
    ...
    folder_key: z
      ...
      .optional()
      .describe(
        "Key ของ Folder จาก setup (เช่น '09_Meeting_Notes/Weekly', '04_Analytics/Monthly_Reports')"
      ),
  },
...
server.tool(
  ...
  "สร้าง Google Doc สรุปบทสนทนาปัจจุบันโดยอัตโนมัติ แล้วบันทึกใน Folder Meeting Notes",
  {
    custom_content: z
      .string()
      .optional()
  ...
  async ({ custom_content }) => {
    ...
    const doc = await docs.createSessionSummaryDoc(
      parentId || "",
      content,
      SESSION_DATE
    );
...
server.tool(
  "upload_to_drive",
  "อัปโหลดไฟล์จาก Repository ขึ้น Google Drive",
  {
    file_path: z.string().describe("Path ของไฟล์ใน Repository"),
    folder_key: z
      .string()
      .describe(
        "Key ของ Folder ปลายทาง (เช่น '01_Marketing_Plan/Annual_Plans')"
      ),
...
server.tool(
  "upload_all_marketing_docs",
  "อัปโหลดเอกสารการตลาดทั้งหมดใน Repository ขึ้น Google Drive ตาม Folder Structure",
  {},
  async () => {
    const { drive } = getServices();
    const idsPath = path.join(__dirname, "..", ".folder-ids.json");
    ...
    const fileMap: Array<{ file: string; folderKey: string }> = [
...
server.tool(
  "list_drive_folder",
  "แสดงรายการไฟล์และ Folder ใน Google Drive",
  {
    folder_key: z
      .string()
      .optional()
...
server.tool(
  "create_audit_report_doc",
  "สร้าง Google Doc สำหรับ AI Audit Report รายสัปดาห์",
  {
